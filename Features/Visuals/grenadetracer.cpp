...
{
	auto WeaponIDz = *weapon->IntGetItemDefinitionIndex();
...
void GrenadePrediction::View(CViewSetup* setup)
{
	...
	if (g::pLocalEntity && g::pLocalEntity->GetHealth() > 0)
	{
		...
		if (IsGrenade(weapon))
		{
			type = *weapon->IntGetItemDefinitionIndex();
			Simulate(setup);
		}
		else
			type = 0;
...
void GrenadePrediction::Paint()
{
	...
	if (g::pLocalEntity->GetHealth() > 0)
	{
		if ((type) && path.size() > 1)
		{
			...
			{
				if (WeaponIDz == 43)
					DrawF((int)nadeEnd.x - 5, (int)nadeEnd.y, g::x88Font, true, true, Color(255, 255, 255, 255), "i");
				else if (WeaponIDz == 44)
					DrawF((int)nadeEnd.x - 5, (int)nadeEnd.y, g::x88Font, true, true, Color(255, 255, 255, 255), "j");
				else if (WeaponIDz == 48)
					DrawF((int)nadeEnd.x - 5, (int)nadeEnd.y, g::x88Font, true, true, Color(255, 255, 255, 255), "n");
				else if (WeaponIDz == 46)
					DrawF((int)nadeEnd.x - 5, (int)nadeEnd.y, g::x88Font, true, true, Color(255, 255, 255, 255), "l");
				else if (WeaponIDz == 47)
					DrawF((int)nadeEnd.x - 5, (int)nadeEnd.y, g::x88Font, true, true, Color(255, 255, 255, 255), "m");
				else if (WeaponIDz == 45)
					DrawF((int)nadeEnd.x - 5, (int)nadeEnd.y, g::x88Font, true, true, Color(255, 255, 255, 255), "k");
			}
		}
	}
}
void GrenadePrediction::Setup(Vector& vecSrc, Vector& vecThrow, Vector viewangles)
{
	...
	vecSrc = g::pLocalEntity->GetEyePosition();
